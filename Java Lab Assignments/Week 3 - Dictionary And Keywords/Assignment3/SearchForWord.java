...
public int Search( String A, String[] Keyword)
{
	
	int mid;
	int low=0,high=Keyword.length-1;
	while(low<=high)
	{
		mid=(high+low)/2;
		if(Keyword[mid].compareTo(A)==0)
		{
			return 0;
		}
		else if(Keyword[mid].compareTo(A)<0)
		{
			low=mid+1;
		}
		else high=mid -1;
...
public static int Compare_strs2(String s1, String s2)
{
	int l1=s1.length();
	int i;
	int l2=s2.length();
