...
// or null otherwise.
    ...
    public T remove() {
        ...
        Random rn=new Random();  //create a random position
        int random=rn.nextInt(numberOfEntries);
        
       
        Node current=null;
        
        //this loop gets the node having the index equal to the random no
        if(random!=0)
        {
        	 current=firstNode;
        	Node prev=null;
        	...
        	//swapping the first and the to-be-removed node
    ...
    public boolean equals(LinkedBag<T> aBag) {
        ...
        //A bag is said to be equal to another bag if both contain same no of elements with the same frequency
        
        
        for(int i=0;i< numberOfEntries;i++)
        {
        	int count1=this.getFrequencyOf(scout.data);   //get the freq of the ith element for the calling object this
        	int count2=aBag.getFrequencyOf(scout.data);   //get the freq of the ith element for the Abag object
    ...
    public void removeDuplicates() {
        ...
        //Creating a new bag to store the contains of the duplicate bag
        LinkedBag<T>newFirst=new LinkedBag<T>();
        while(temp!=null)
        {
        	if(newFirst.getFrequencyOf(temp.data)==0)
        	{
        		newFirst.add(temp.data);
        	}
        	temp=temp.next;
        }
