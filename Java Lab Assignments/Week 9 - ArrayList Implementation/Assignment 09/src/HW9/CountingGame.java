...
public class CountingGame
{
    ...
    public static void main(String args[])
    {
        ...
        int max;
        int position = 0;       // always start with the first player
        ...
        AList<Integer> pList = new AList<>(max);
        players = pList;
        for(int i = 0; i < max; i++)
        	players.add(i + 1);
   ...
   public static int doRhyme(ListInterface<Integer> players, ListInterface<String> rhyme, int startAt)
    {
	   ...
	   for(i = 1; i <= rhyme.getLength(); i++)
		   System.out.println("Player " + players.getEntry((startAt + i - 1) % players.getLength() +1 ) + " : " + rhyme.getEntry(i));
	   
	   i--; 
	   int position = (startAt + i - 1) % players.getLength() +1 ;
	   System.out.println("Removing player " + players.getEntry(position));
	   
	   // Removing the player.
	   players.remove(position);
	   
	   // Returning the new position to start the chant at.
        return position;
     }
